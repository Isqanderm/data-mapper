...
используйте разные `constraints`. Вынести объект-валидатор наружу из `addInitializer` тоже полезно:
это делает дедупликацию между созданиями экземпляров точной (по ссылке), а не основанной на исходном
тексте.
