...
export type DeepPath<Source, K extends keyof Source> = K extends string ? IsObjectWithProperties<Source[K]> extends true ? IsArray<Source[K]> extends true ? K | `${K}.[]` | `${K}.[${number}]` | `${K}.[${number}].${DeepPath<ExtractArrayType<Source[K]>, keyof ExtractArrayType<Source[K]>>}` | `${K}.[].${DeepPath<ExtractArrayType<Source[K]>, keyof ExtractArrayType<Source[K]>>}` : K | `${K}.${DeepPath<Source[K], keyof Source[K]>}` : K : never;
